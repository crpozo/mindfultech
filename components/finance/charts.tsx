...
function Tooltip({ hover, width }: { hover: Hover; width: number }) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: 11, color: MUTED, fontFamily: "var(--mono)", letterSpacing: ".06em", textTransform: "uppercase" }}>
        {hover.title}
      </div>
      {hover.rows.map((r) => (
        <div key={r.label} style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 5 }}>
          {r.color && (
            <span style={{ width: 8, height: 8, borderRadius: 2, background: r.color, flex: "none" }} />
          )}
          <span style={{ fontSize: 12.5, color: "#52514e", flex: 1 }}>{r.label}</span>
          <span style={{ fontSize: 12.5, color: INK, fontVariantNumeric: "tabular-nums" }}>{r.value}</span>
        </div>
      ))}
    </div>
  );
 ...
 */
export function TrendChart({ months, lang }: { months: MonthSummary[]; lang: Lang }) {
  ...
  const maxVal = niceMax(Math.max(1, ...data.flatMap((m) => [m.income, m.expense, m.net])));
  ...
  // encima del cero).
  const ticks = (minVal < 0 ? [0, maxVal, minVal] : [0, maxVal, maxVal / 2]).reduce<number[]>(
    (kept, t) => (kept.some((k) => Math.abs(y(k) - y(t)) < 16) ? kept : [...kept, t]),
    []
  );
  const netPoints = data.map((m, i) => [pad.left + slot * (i + 0.5), y(m.net)] as const);

  return (
    <div ref={ref} style={{ position: "relative" }}>
      <Legend
        items={[
          { label: lang === "es" ? "Ingresos" : "Income", color: SERIES.income },
          { label: lang === "es" ? "Gastos" : "Expenses", color: SERIES.expense },
          { label: lang === "es" ? "Neto" : "Net", color: SERIES.net, line: true },
        ]}
      />
      {width > 0 && (
        <svg
          ...
          height={H}
          role="img"
          aria-label={lang === "es" ? "Ingresos, gastos y neto por mes" : "Income, expenses and net by month"}
          style={{ display: "block", overflow: "visible" }}
        >
          {ticks.map((t) => (
            <g key={t}>
              <line x1={pad.left} x2={w - pad.right} y1={y(t)} y2={y(t)} stroke={t === 0 ? AXIS : GRID} strokeWidth={1} />
              <text x={pad.left - 8} y={y(t) + 4} textAnchor="end" fontSize={10.5} fill={MUTED} fontFamily="var(--mono)">
                {fmtShort(t)}
              </text>
            </g>
          ))}
          ...
          {data.map((m, i) => {
            ...
            return (
              <g key={m.month}>
                <path d={barPath(cx - barW - gap / 2, y(m.income), barW, incH)} fill={SERIES.income} />
                <path d={barPath(cx + gap / 2, y(m.expense), barW, expH)} fill={SERIES.expense} />
                {/* Etiqueta directa solo en el mes más reciente: es la lectura que
                    importa, y cumple la regla de relieve para el aqua. */}
                {last && m.income > 0 && (
                  <text x={cx - barW / 2 - gap / 2} y={y(m.income) - 6} textAnchor="middle" fontSize={10.5} fill={INK} style={{ fontVariantNumeric: "tabular-nums" }}>
                    {fmtShort(m.income)}
                  </text>
                )}
                {last && m.expense > 0 && (
                  <text x={cx + barW / 2 + gap / 2} y={y(m.expense) - 6} textAnchor="middle" fontSize={10.5} fill={INK} style={{ fontVariantNumeric: "tabular-nums" }}>
                    {fmtShort(m.expense)}
                  </text>
                )}
                <text x={cx} y={H - 8} textAnchor="middle" fontSize={10.5} fill={MUTED} fontFamily="var(--mono)">
          ...
          {netPoints.map(([px, py], i) => (
            ...
            <circle key={i} cx={px} cy={py} r={4} fill={SERIES.net} stroke={SURFACE} strokeWidth={2} />
          ))}
              ...
              <rect
                ...
                onMouseEnter={() =>
                  setHover({
                    ...
                    rows: [
                      { label: lang === "es" ? "Ingresos" : "Income", value: fmtMoney(m.income, lang), color: SERIES.income },
                      { label: lang === "es" ? "Gastos" : "Expenses", value: fmtMoney(m.expense, lang), color: SERIES.expense },
                      { label: lang === "es" ? "Neto" : "Net", value: fmtMoney(m.net, lang), color: SERIES.net },
                    ],
                  })
                }
...
export function CategoryBars({
  ...
  const rows = rest > 0 ? [...head, ["__rest__", rest] as [string, number]] : head;
  ...
  if (!rows.length) {
    return (
      <p style={{ fontSize: 13.5, color: MUTED, margin: "8px 0 0" }}>
        {lang === "es" ? "Sin gastos registrados este mes." : "No expenses recorded this month."}
      </p>
    );
  }
        ...
        return (
          <div key={cat} title={`${label} · ${pct.toFixed(1)}%`}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10, marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 12.5, color: INK, fontVariantNumeric: "tabular-nums" }}>
                {fmtMoney(value, lang, currency)}
                <span style={{ color: MUTED, marginLeft: 6 }}>{pct.toFixed(0)}%</span>
              </span>
            </div>
...
export function HealthGauge({ score, verdict, lang }: { score: number; verdict: string; lang: Lang }) {
  ...
  const color =
    pct >= 81 ? "#0ca30c" : pct >= 61 ? "#69c7b9" : pct >= 41 ? "#fab219" : "#d03b3b";
  ...
  const text = labels[verdict] ? (lang === "es" ? labels[verdict].es : labels[verdict].en) : verdict;

  return (
    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
      <svg width={size} height={size} role="img" aria-label={`${score} / 100 — ${text}`}>
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="#f1f2f6" strokeWidth={stroke} />
        <circle
          cx={size / 2}
          cy={size / 2}
          r={r}
        ...
        />
        <text x={size / 2} y={size / 2 + 2} textAnchor="middle" fontSize={26} fill={INK} fontWeight={500}>
          {Math.round(score)}
        </text>
        <text x={size / 2} y={size / 2 + 20} textAnchor="middle" fontSize={10} fill={MUTED} fontFamily="var(--mono)">
          / 100
        </text>
      </svg>
      <div>
        <div style={{ fontFamily: "var(--mono)", fontSize: 10.5, letterSpacing: ".2em", color: MUTED, textTransform: "uppercase" }}>
          {lang === "es" ? "Salud financiera" : "Financial health"}
        </div>
        <div style={{ fontSize: 20, fontWeight: 500, color: INK, marginTop: 4 }}>{text}</div>
      </div>
    </div>
  );
}
